...
class HideScreen extends React.Component {
  constructor(props) {
    ...
    let toggle = this.user.date === '상시';

    const image_data = {
      name: this.user.image,
      type: 'image/jpeg',
      uri: this.user.url,
    };
      ...
      <View>
        <View style={styles.textInputContainer}>
          ...
          <TextInput
            ...
            value={this.state.date === '상시' ? '' : this.state.date}
          />
        </View>
        <TouchableOpacity
          style={styles.inputButtonContainer}
  ...
  checkValidate = () => {
    ...
    if (this.state.title === '') {
      ...
      this.setState({
        modalVisible: true,
        modalText: '이미지는 초기화 됩니다. 이미지를 다시 선택한 후',
      });
    } else {
      validate = true;
    }
  ...
  send = navigation => {
    ...
    return axios
      .put('http://101.101.208.57:8080/api/perform/' + this.user.title, {
...
const styles = StyleSheet.create({
  ...
  get_front: {zIndex: 1},
  get_back: {zIndex: 3},
  backColor: {backgroundColor: 'rgba(0, 0, 0, 0.3)'},
  clear: {backgroundColor: 'rgba(0, 0, 0, 0.0)'},
});
